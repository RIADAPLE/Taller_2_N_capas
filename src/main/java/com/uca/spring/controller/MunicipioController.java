...
public class MunicipioController {
  ...
  public @ResponseBody List<CboFilter> cbofilterMunicipioByDep(
    ...
    for (int i = 0; i < listMunicipio.size(); i++) {
      response.add(new CboFilter(Integer.toString(listMunicipio.get(i).getIdMunicipio()),
  ...
  List<CboFilter> cbofilterMunicipio() {
    List<Municipio> municipios = municipioRepository.findAll();
      ...
      for (int i = 0; i < municipios.size(); i++) {
          response.add(new CboFilter(Integer.toString(municipios.get(i).getIdMunicipio()), municipios.get(i).getDescripcion()));
      }
      return response;
  }
